...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "KarmaBot"
...
async def _(event):
    ...
    if not event.is_reply:
        await event.edit("Reply to a photo for adding to my personal sticker pack.")
        return
    ...
    #packname = f"KarmaKeStickers"
    #packshortname = f"KarmaBot_{userid}_ns"  # format: Uni_Borg_userid
    if userid == 729596789:
        packname = f"KarmaKeStickers"
        packshortname = "Karma_Ke_locker_me"
    else:
        packname = f"{DEFAULTUSER}'s KarmaBot Vol.{pack}"
        packshortname = f"KarmaBot_{userid}_kang"
    ...
    if is_a_s:
        ...
        if userid == 729596789:
            packname = f"KarmaKeAnimeted"
            packshortname = "Karma_Ke_Animated"
        else:
            packname = f"{DEFAULTUSER}'s KarmaBot Animated Vol.{pack}"
            packshortname = f"KarmaBot_{userid}" # format: Uni_Borg_userid
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    ...
                    #if userid == 729596789:
                       # packname = f"{user.first_name}'s KarmaBot Vol.{pack}"
                       # packshortname = "Vol._{pack}_Karma_ke_locker_me"
                   # else:
                       # packname = f"Vol._{pack}_KarmaBot{userid}"
                        #packshortname = f"Vol._{pack}_KarmaBot_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
